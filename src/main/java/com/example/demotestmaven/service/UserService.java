...
public class UserService {
        ...
        .orElseThrow(
            () ->
                new ApiException(
                    ...
                    username)); // BusinessException(ErrorCode.USER_NOT_FOUND, username));
  }

  // Get users by createdAt before a given date time

  @Transactional(readOnly = true)
  public List<UserDTO> getUsersByCreatedAtBeforeAndAfter(
      String currentUsername, String timeBefore, String timeAfter) {
  ...
  public List<UserExcelFullResponseDTO> importUsersFromExcel(MultipartFile file) {
    ...
    for (UserExcelRequestDTO userExcelRequestDTO : userExcelRequestDTOs) {
      ...
      if ((userExcelRequestDTO.getUsername() == null)
          || (userExcelRequestDTO.getEmail() == null)
          || (userExcelRequestDTO.getRolecodes() == null)
          || (userExcelRequestDTO.getPassword() == null)
      ...
      UserExcelFullResponseDTO userExcelFullResponseDTO =
          transformUserExcelRequestDTO(userExcelRequestDTO);
  ...
  public List<UserResponseDTO> createUsers(
      String currentUsername, List<UserRequestDTO> userRequestDTOs) {
    if (!isCurrentUserAdmin(currentUsername)) {
    ...
    List<User> users = userRequestDTOs.stream().map(this::createUser).collect(Collectors.toList());
    ...
    return userResponseDTOs.stream()
        .filter(
            userResponseDTO ->
                (userResponseDTO.getMessages() != GlobalConstants.USER_SUCCESS_MESSAGE))
        .collect(Collectors.toList());
  }

  @Transactional
  public Mono<UserResponseBatch> createUsersBatchAsync(
      String currentUsername, List<UserRequestDTO> userRequestDTOs) {
    if (!isCurrentUserAdmin(currentUsername)) {
      throw new ApiException(ApiErrorType.FORBIDDEN_OPERATION);
    }
                  ...
                  () -> {
                    try {
                      if (userItemResponseDto.getStatus().equals(GlobalConstants.SUCCESS_STATUS)) {
                        saveUserWithErrorHandling(user);
                        logger.info(
                            "Thread {} completed saving user: {}",
                            Thread.currentThread().getName(),
                            user.getUsername());
                      }
                    } catch (Exception e) {
                      logger.error(
                          "Thread {} failed to save user: {} with error: {}",
                          Thread.currentThread().getName(),
                          user.getUsername(),
                          e.getMessage());
                      ...
                      responseDTOs.stream()
                          .filter(
                              userItemResponseDto ->
                                  userItemResponseDto
                                      .getStatus()
                                      .equals(GlobalConstants.SUCCESS_STATUS))
                          .count());
              batchResponse.setFailureCount(
                  (int)
                      responseDTOs.stream()
                          .filter(
                              userItemResponseDto ->
                                  userItemResponseDto
                                      .getStatus()
                                      .equals(GlobalConstants.ERROR_STATUS))
                          .count());
              batchResponse.setSuccessRate(
                  ((float) batchResponse.getSuccessCount()) / responseDTOs.size() * 100);
  ...
  @Transactional
  public UserResponseBatchSuccessErrorDto createUsersBatchSuccessError(
      String currentUsername, List<UserRequestDTO> userRequestDTOs) {
    if (!isCurrentUserAdmin(currentUsername)) {
    ...
    for (UserRequestDTO userRequestDTO : userRequestDTOs) {
      ...
      UserItemResponseDto userItemResponseDto = createUserItemResponseDto(user);
      if (userItemResponseDto.getStatus().equals(GlobalConstants.SUCCESS_STATUS)) {
        userBatchSuccessfulDtos.add(
            new UserBatchSuccessfulDto(user.getUsername(), user.getEmail(), user.getPassword()));
        saveUserWithErrorHandling(user);
        logger.info(
            "Thread {} completed saving user: {}",
            Thread.currentThread().getName(),
        ...
        logger.info(
            ...
            Thread.currentThread().getName(),
            user.getUsername(),
            userItemResponseDto.getMessage());
        userBatchErrorDtos.add(
            new UserBatchErrorDto(user.getUsername(), userItemResponseDto.getMessage()));
      }
    }
    UserResponseBatchSuccessErrorDto userResponseBatchSuccessErrorDto =
    ...
    userResponseBatchSuccessErrorDto.setSuccessRate(
        ((float) userBatchSuccessfulDtos.size()) / userRequestDTOs.size() * 100);
  ...
  private UserItemResponseDto createUserItemResponseDto(User user) {
    ...
    if ((user.getUsername() == null) || (user.getUsername().isEmpty())) {
      ...
      userItemResponseDto.setMessage(
          String.format(ApiErrorType.USER_ALREADY_EXISTS.getMessage(), user.getUsername()));
    } else if (userRepository.findByEmail(user.getEmail()).isPresent()) {
      userItemResponseDto.setMessage(
          String.format(ApiErrorType.USER_EMAIL_ALREADY_EXISTS.getMessage(), user.getEmail()));
    ...
    userItemResponseDto.setStatus(
        userItemResponseDto.getMessage().equals(GlobalConstants.USER_SUCCESS_MESSAGE)
            ? GlobalConstants.SUCCESS_STATUS
            : GlobalConstants.ERROR_STATUS);
    return userItemResponseDto;
  }
  ...
  private List<UserExcelRequestDTO> getUserExcelRequestDTOs(MultipartFile file) {
    try {
      ...
      for (Row row : sheet) {
        ...
        UserExcelRequestDTO userExcelRequestDTO =
            convertToUserExcelRequestDTO(row, excelMappingHeader);
  ...
  private UserExcelRequestDTO convertToUserExcelRequestDTO(
      Row row, Map<String, Integer> excelMappingHeader) {
    UserExcelRequestDTO userExcelRequestDTO = new UserExcelRequestDTO();
    userExcelRequestDTO.setRowNumber(
        getCellValuefromExcelMappingHeader(row, excelMappingHeader, ROW_NUMBER_HEADER));
    userExcelRequestDTO.setUsername(
        getCellValuefromExcelMappingHeader(row, excelMappingHeader, USERNAME_HEADER));
    userExcelRequestDTO.setPassword(
        getCellValuefromExcelMappingHeader(row, excelMappingHeader, PASSWORD_HEADER));
    userExcelRequestDTO.setEmail(
        getCellValuefromExcelMappingHeader(row, excelMappingHeader, EMAIL_HEADER));
    userExcelRequestDTO.setRolecodes(
        getCellValuefromExcelMappingHeader(row, excelMappingHeader, ROLECODE_HEADER));
    userExcelRequestDTO.setRoletypes(
        getCellValuefromExcelMappingHeader(row, excelMappingHeader, ROLETYPE_HEADER));
    return userExcelRequestDTO;
  }

  private String getCellValuefromExcelMappingHeader(
      Row row, Map<String, Integer> excelMappingHeader, String header) {
  ...
  private boolean validateUserExcelRequestDTO(
      UserExcelRequestDTO userExcelRequestDTO, Map<String, User> users) {
  ...
  private boolean validateExistingUserExcelImport(
      User existingUser, UserExcelRequestDTO userExcelRequestDTO) {
  ...
  private UserExcelFullResponseDTO transformUserExcelRequestDTO(
      UserExcelRequestDTO userExcelRequestDTO) {
    ...
    // Check empty fields
    List<String> messagesForCheckingEmptyFields =
        getMessagesForCheckingEmptyFields(userExcelRequestDTO);
    ...
    List<String> messagesForCheckingUserUniqueness =
        getMessagesForCheckingUserUniqueness(userExcelRequestDTO);
    ...
    List<String> messagesForCheckingEmailAndPassword =
        getMessagesForCheckingEmailAndPassword(userExcelRequestDTO);
    ...
    fields.forEach(
        (fieldHeader, fieldValue) -> {
          if (fieldValue.isEmpty()) {
            messages.add(
                String.format(
                    ApiErrorType.USER_EXCEL_MISSING_DATA.getFormattedMessage(fieldHeader)));
          }
        });
    return messages;
  }
  ...
  private List<String> getMessagesForCheckingUserUniqueness(UserExcelRequestDTO requestDTO) {
    ...
    if (userRepository.findByUsername(requestDTO.getUsername()).isPresent()) {
      messages.add(
          String.format(
              ApiErrorType.USER_ALREADY_EXISTS.getFormattedMessage(requestDTO.getUsername())));
    }
    if (userRepository.findByEmail(requestDTO.getEmail()).isPresent()) {
      messages.add(
          String.format(
              ApiErrorType.USER_EMAIL_ALREADY_EXISTS.getFormattedMessage(requestDTO.getEmail())));
    }
    return messages;
  }
  ...
  private List<String> getMessagesForCheckingEmailAndPassword(UserExcelRequestDTO requestDTO) {
    ...
    if (!requestDTO.getPassword().isEmpty() && requestDTO.getPassword().length() < 8) {
      messages.add(
          String.format(
              ApiErrorType.USER_INVALID_PASSWORD.getFormattedMessage(requestDTO.getPassword())));
    }
    if (!requestDTO.getEmail().isEmpty()
        && !requestDTO.getEmail().matches("^[A-Za-z0-9+_.-]+@(.+)$")) {
      messages.add(
          String.format(
              ApiErrorType.USER_INVALID_EMAIL.getFormattedMessage(requestDTO.getEmail())));
    }
    return messages;
  }
  ...
  private UserExcelFullResponseDTO validateExistingExcelUser(
      User existingUser, UserExcelRequestDTO userExcelRequestDTO) {
  ...
  private UserExcelFullResponseDTO validateNewExcelUser(UserExcelRequestDTO userExcelRequestDTO) {
    ...
    if (userRepository.findByUsername(userExcelRequestDTO.getUsername().trim()).isPresent()) {
      messages.add(
          String.format(
              ApiErrorType.USER_ALREADY_EXISTS.getMessage(), userExcelRequestDTO.getUsername()));
      messages.set(0, VALID_ERROR);
    }
    if (userRepository.findByEmail(userExcelRequestDTO.getEmail().trim()).isPresent()) {
      messages.add(
          String.format(
              ApiErrorType.USER_EMAIL_ALREADY_EXISTS.getMessage(),
              userExcelRequestDTO.getEmail(),
              userExcelRequestDTO.getEmail()));
      messages.set(0, VALID_ERROR);
    }
    ...
    if (userExcelRequestDTO.getPassword().length() < 8) {
      messages.add(
          String.format(
              ApiErrorType.USER_INVALID_PASSWORD.getMessage(), userExcelRequestDTO.getPassword()));
      messages.set(0, VALID_ERROR);
    }
    if (!userExcelRequestDTO.getEmail().matches("^[A-Za-z0-9+_.-]+@(.+)$")) {
      messages.add(
          String.format(
              ApiErrorType.USER_INVALID_EMAIL.getMessage(), userExcelRequestDTO.getEmail()));
      messages.set(0, VALID_ERROR);
    }
    if (userExcelRequestDTO.getRolecodes().isEmpty()) {
      messages.add(
          String.format(
              ApiErrorType.USER_ROLE_REQUIRED.getMessage(), userExcelRequestDTO.getUsername()));
      messages.set(0, VALID_ERROR);
    }
    if (userExcelRequestDTO.getRoletypes().isEmpty()) {
      messages.add(
          String.format(
              ApiErrorType.USER_ROLE_TYPE_INVALID.getMessage(), userExcelRequestDTO.getUsername()));
      messages.set(0, VALID_ERROR);
    }
    ...
    if (userExcelRequestDTO.getRolecodes() == null) {
      messages.add(
          String.format(
              ApiErrorType.USER_ROLE_CODE_INVALID.getMessage(), userExcelRequestDTO.getUsername()));
      messages.set(0, VALID_ERROR);
    }
    ...
    if (userExcelRequestDTO.getRolecodes() != null && userExcelRequestDTO.getRoletypes() != null) {
      ...
      if (!StringUtils.hasText(userExcelRequestDTO.getRolecodes())) {
        messages.add(
            String.format(
                ApiErrorType.USER_ROLE_CODE_INVALID.getMessage(),
                userExcelRequestDTO.getUsername()));
        messages.set(0, VALID_ERROR);
      }
      if (!StringUtils.hasText(userExcelRequestDTO.getRoletypes())) {
        messages.add(
            String.format(
                ApiErrorType.USER_ROLE_TYPE_INVALID.getMessage(),
                userExcelRequestDTO.getUsername()));
        messages.set(0, VALID_ERROR);
      }
    }
  ...
  private User saveUserWithErrorHandling(User user) {
    try {
      ...
      logger.error(
          "Connection failure while saving user {}: {}", user.getUsername(), e.getMessage());
