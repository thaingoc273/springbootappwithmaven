...
public class UserController {
  ...
  @Autowired private CityPopulationService cityPopulationService;
  ...
  public ResponseEntity<List<UserDTO>> getUsersByCreatedAtBeforeAndAfter(
      @RequestHeader("X-Current-User") String currentUsername,
      @RequestParam(required = false, defaultValue = "2099-01-01", value = "before")
          String beforeDate,
      @RequestParam(required = false, defaultValue = "1999-01-01", value = "after")
          String afterDate) {
    return ResponseEntity.ok(
        userService.getUsersByCreatedAtBeforeAndAfter(currentUsername, beforeDate, afterDate));
  }

  @PostMapping("/")
  public ResponseEntity<UserDTO> createUser(
      @RequestHeader("X-Current-User") String currentUsername, @RequestBody UserDTO userDTO) {
  ...
  public Mono<ResponseEntity<UserResponseBatch>> createUsersBatchAsync(
      ...
      @RequestBody List<UserRequestDTO> userRequestDTOs) {
    return userService
        .createUsersBatchAsync(currentUsername, userRequestDTOs)
        .map(
            result -> {
  ...
  public ResponseEntity<List<UserExcelFullResponseDTO>> importUsersFromExcel(
  ...
  public ResponseEntity<UserResponseBatchSuccessErrorDto> createUsersBatchSuccessError(
      ...
      @RequestBody List<UserRequestDTO> userRequestDTOs) {
    try {
      UserResponseBatchSuccessErrorDto result =
          userService.createUsersBatchSuccessError(currentUsername, userRequestDTOs);
      if (result.getSuccessRate() >= 80) {
        return ResponseEntity.ok(result);
      } else {
        return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(result);
