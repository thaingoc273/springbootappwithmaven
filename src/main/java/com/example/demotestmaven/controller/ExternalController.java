...
public class ExternalController {
  ...
  public ResponseEntity<?> callExternalAPI_getPopulationByCities() {
    try {
      return ResponseEntity.ok(
          cityPopulationService.callExternalAPI_getPopulationByCitiesUsingObjectMapper());
    } catch (JsonProcessingException e) {
      return ResponseEntity.internalServerError()
          .body("Error processing JSON response: " + e.getMessage());
    }
  }

  @GetMapping("/webclient/population/cities")
  public Mono<ResponseEntity<CityPopulationResponseDTO>>
      callExternalAPI_getPopulationByCitiesUsingWebClient() {
    return cityPopulationService
        .callExternalAPI_getPopulationByCitiesUsingWebClient()
  ...
  public Mono<ResponseEntity<CityPopulationResponseDTO>>
      callExternalAPI_getPopulationByCitiesUsingWebClient_Version2() {
    return cityPopulationService
        .callExternalAPI_getPopulationByCitiesUsingWebClient_Version2()
        .map(ResponseEntity::ok)
        .onErrorResume(
