...
class UserServiceTest {
  ...
  @Mock private AsyncUserService asyncUserService;
  ...
  void getUsersByCreatedAtBefore_WhenNormal_ShouldReturnListOfUsers() {
    // Arrange
    when(roleRepository.findByUser_Username(adminUsername)).thenReturn(Arrays.asList(adminRole));
    when(userRepository.findByCreatedAtBeforeAndAfter(
            convertToLocalDateTime(dateBefore), convertToLocalDateTime(dateAfter)))
        .thenReturn(Arrays.asList(testUser));

    // Act
    List<UserDTO> result =
        userService.getUsersByCreatedAtBeforeAndAfter(adminUsername, dateBefore, dateAfter);
        ...
        assertThrows(
            ...
            () ->
                userService.getUsersByCreatedAtBeforeAndAfter(
                    currentUsername, "invalid-date", "2025-02-20"));
  ...
  void importUsersFromExcel_WhenDuplicateUsername_ShouldReturnError() throws Exception {
    ...
    // Arrange
    MultipartFile file =
        getMockMultipartFile(TestConstants.USER_EXCEL_IMPORT_DUPLICATE_USERNAME_FILE);
    // when(passwordEncoder.encode(anyString())).thenReturn("encodedPassword");
    ...
    String expectedMessage =
        ApiErrorType.USER_ALREADY_EXISTS.getFormattedMessage(testUser.getUsername());
  ...
  void importUsersFromExcel_WhenDuplicateEmail_ShouldReturnError() throws Exception {
    ...
    assertEquals(
        ApiErrorType.USER_EMAIL_ALREADY_EXISTS.getFormattedMessage(testUser.getEmail()),
        result.get(0).getMessages().get(0));
  }

  @Test
  void importUsersFromExcel_WhenMissingUsername_ShouldReturnMissingUsernameError()
      throws Exception {
    // Arrange
    MultipartFile file =
        getMockMultipartFile(TestConstants.USER_EXCEL_IMPORT_MISSING_USERNAME_FILE);
    // when(passwordEncoder.encode(anyString())).thenReturn("encodedPassword");
  ...
  @Test
  void importUsersFromExcel_WhenMissingPassword_ShouldReturnInvalidPasswordError()
      throws Exception {
    // Arrange
    MultipartFile file =
        getMockMultipartFile(TestConstants.USER_EXCEL_IMPORT_MISSING_PASSWORD_FILE);
    // when(passwordEncoder.encode(anyString())).thenReturn("encodedPassword");
    ...
    List<String> actualMessages = result.get(0).getMessages();
    String expectedMessage =
        ApiErrorType.USER_EXCEL_MISSING_DATA.getFormattedMessage(GlobalConstants.PASSWORD_HEADER);
  ...
  void importUsersFromExcel_WhenMissingEmail_ShouldReturnInvalidEmailError() throws Exception {
    ...
    List<String> actualMessages = result.get(0).getMessages();
    String expectedMessage =
        ApiErrorType.USER_EXCEL_MISSING_DATA.getFormattedMessage(GlobalConstants.EMAIL_HEADER);
  ...
  @Test
  void importUsersFromExcel_WhenMissingRoleCode_ShouldReturnMissingRoleCodeError()
      throws Exception {
    // Arrange
    MultipartFile file =
        getMockMultipartFile(TestConstants.USER_EXCEL_IMPORT_MISSING_ROLE_CODE_FILE);
    ...
    List<String> actualMessages = result.get(0).getMessages();
    String expectedMessage =
        ApiErrorType.USER_EXCEL_MISSING_DATA.getFormattedMessage(GlobalConstants.ROLECODE_HEADER);
  ...
  @Test
  void importUsersFromExcel_WhenFalseStructureFile_ShouldReturnInvalidStructureFileError()
      throws Exception {
    // Arrange
  ...
  private LocalDateTime convertToLocalDateTime(String time) {
    try {
      ...
      throw new ValidationException(
          String.format("Invalid date time format, please use yyyy-MM-dd"));
    }
  }
